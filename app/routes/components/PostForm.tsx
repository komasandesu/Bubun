...
const PostForm: React.FC = () => {
    ...
    return (
        ...
        >
            {/* 上段：○○の */}
            <div className="flex items-center mb-4 w-full">  {/* 枠線だけ残す */}
                <textarea
                    name="originalString"
                    required
                    rows={1}
                    className="border border-gray-300 rounded p-2 flex-1"  // 枠線のみ
                    placeholder="○○"
                    value={originalString}
                    onChange={(e) => setOriginalString(e.target.value)}
                />
                <span className="text-gray-500 ml-2 whitespace-nowrap">の</span>
            </div>
            ...
            <div className="flex items-center mb-4 w-full"> {/* 枠線だけ残す */}
                <textarea
                    ...
                    required
                    rows={1}
                    className="border border-gray-300 rounded p-2 flex-1 resize-y"  // 枠線のみ
                    placeholder="××"
                    value={substring}
